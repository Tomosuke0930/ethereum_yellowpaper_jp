...
世界中のほとんどの場所でインターネットに接続できるようになり、グローバルな情報発信が驚くほど安価にできるようになりました。

ビットコインのような技術に根ざした動きは、デフォルトの力、コンセンサスメカニズム、社会契約の自発的な尊重を通じて、インターネット　を使って、世界中で共有でき、事実上無料で使える分散型の価値伝達システムを作ることが可能であることを実証した。

このシステムは、暗号学的に安全な、トランザクションベースのステートマシンの非常に特殊なバージョンであると言うことができる。

その後、Namecoinのようなシステムが、この当初の「通貨アプリケーション」の技術をかなり単純なものではあるが、他の用途に応用している。

イーサリアムは、すべてのトランザクションベースのステートマシンの概念を構築することができる汎用的な技術を構築しようとするプロジェクトである。

さらに、信頼性の高いオブジェクトメッセージング・コンピューティングフレームワークという、これまで主流ではなかったコンピューティングパラダイムでソフトウェアを構築するための、緊密に統合されたエンドツーエンドシステムをエンド開発者に提供することを目的としています。

\subsection{Driving Factors} \label{ch:driving}

このプロジェクトにはさまざまな目的がありますが、重要な目的のひとつは、互いに**信頼する術を持たない、**同意した個人間の取引を円滑にすることです。

これは、地理的な隔たり、インターフェースの難しさ、あるいは既存の法体系の非互換性、無能力、不本意、費用、不確実性、不便、腐敗などが原因かもしれません。

豊かで曖昧さのない言語によって状態変化システムを指定し、さらに、合意が自律的に執行されることを合理的に期待できるようなシステムを構築することによって、この目的のための手段を提供することができる。

この提案されたシステムでの取引は、現実世界ではあまり見られないいくつかの属性を持つことになる。判断の無節操さは，しばしば見つけるのが難しいが，利害関係のないアルゴリズム解釈者から自然に得られる．透明性、つまりある状態や判断がどのようにして生じたかをトランザクションログやルールや命令コードを通して正確に見ることができることは、人間ベースのシステムでは決して完璧には起こりえないことである
自然言語は曖昧であり、情報は不足しがちであり、また古くからの偏見も拭い去れないからである。

全体として、私たちは、ユーザーが他のどの個人、システム、組織とやりとりする場合でも、次のようなことが保証されるようなシステムを提供したいと願っています。
可能な結果と、その結果がどのようにもたらされるかについて、絶対的な確信を持って行うことができるようなシステムを提供したいと考えています。

\subsection{Previous Work} \label{ch:previous}

% \cite{buterin2013ethereum} は、2013年11月末にこの研究のカーネルを初めて提案しました。現在では多くの点で進化していますが、チューリング完全言語と事実上無制限のトランザクション間保存能力を持つブロックチェーンの主要な機能は変わっていません。
...
% DworkとNaor[1992]は、計算の暗号学的証明の使用に関する最初の研究を提供しました。
...
% \begin{eqnarray}
  ブテリン [2013a]は、2013年11月末にこの研究のカーネルを初めて提案しました。現在では多くの点で進化していますが、チューリング完全言語と事実上無制限のトランザクション間保存能力を持つブロックチェーンの主要な機能は変わっていません。

  DworkとNaor[1992]は、計算の暗号学的証明の使用に関する最初の研究を提供しました。
  Dwork と Naor [1992]は、インターネット上で価値信号を伝送する手段として、暗号による計算支出の証明（「プルーフ・オブ・ワーク」）の使用に関する最初の研究を提供しました。この価値信号は、通貨としてではなく、スパム抑止メカニズムとして利用されたが、基本的なデータチャネルが強力な経済信号を伝送する可能性を決定的に示し、受信者が信頼に頼ることなく物理的な主張をすることを可能にするものであった。Back [2002]は、後に同じような系統のシステムを作成した。
  
  通貨を確保するための強力な経済シグナルとしてプルーフ・オブ・ワークを活用した最初の例は、Vishnumurthy ら [2003]によるものであった。この例では、トークンはピアツーピアのファイル取引を維持するために使用され、「消費者」が「供給者」に対してサービスに対するマイクロペイメントを行う能力を提供しました。プルーフ・オブ・ワークが提供するセキュリティ・モデル
  このプルーフ・オブ・ワークのセキュリティモデルは、電子署名と台帳によって強化され、履歴記録が破損しないように、また悪意のある行為者が支払いを偽装したり、サービス提供に対して不当に文句を言ったりできないようにするために利用されました。その5年後、Nakamoto [2008]は、このようなプルーフ・オブ・ワークで保護された、より広範囲なバリュー・トークンを発表した。
  
  このプロジェクトの成果であるビットコインは、世界的に広く採用された最初の分散型取引台帳となった。
  
  ビットコインの成功を受けて、他のプロジェクトが立ち上がりました。アルトコインは、プロトコルを変更することで、他の多くの通貨を導入しました。最もよく知られているのは、Sprankel [2013]が論じたLitecoinとPrimecoinです。
  
  Aron [2012]は、分散型名前解決システムを提供することを目的としたNamecoinプロジェクトについて論じています。
  
  その他のプロジェクトは、ビットコインのネットワークそのものを構築することを目的としており、ビットコインのシステム内にある大量の価値と、コンセンサスメカニズムにかかる膨大な計算量を利用しています。
  このようなプロジェクトもあります。
  
  Willett氏[2013]によって最初に提案されたMastercoinプロジェクトは、よりリッチなプロトコルを構築することを目的としています。
  コアプロトコルの補助的な部分を利用することで、ビットコインプロトコルの上に多くのハイレベルな機能を追加した、よりリッチなプロトコルを構築することを目的としています。Rosenfeld et al.
  
  [2012]によって提案されたColoured Coinsプロジェクトは、似たような、しかしより単純化された戦略を取り、ファンジョナルを破るために取引のルールを装飾しています。
  ビットコインの基本通貨の換金性をなくし、特別な「クロマウォレット」プロトコルを認識するソフトウェアを通じてトークンの作成と追跡を可能にするために、取引のルールを装飾しています。
  
  Boutellier and Heinzen [2014]によって議論されたRippleは、通貨交換のための「連合」システムを作ろうとし、事実上新しい金融決済システムを作ろうとしています。分散化の前提を捨てれば、高い効率化が可能であることを実証している。
  
  スマートコントラクトに関する初期の研究は、Szabo [1997]とMiller [1997]が行っている。1990年代には、アルゴリズムによる契約の強制が人間の協力関係において大きな力になり得ることが明らかになった。
  
  そのようなシステムを実現するための具体的なシステムは提案されていなかったが、その際、そのようなシステムによって法律の未来が大きく左右されることが提唱された。このような観点から、イーサリアムはそのような暗号法システムの一般的な実装と見なすことができるだろう。
  
  本稿で使用した用語の一覧は、付録 A を参照。
