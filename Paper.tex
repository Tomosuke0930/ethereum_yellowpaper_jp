...
Ethereumは全体として、トランザクションベースのステートマシンと見なすことができる。創世記の状態から始まり、それをある現在の状態に変化させるためにトランザクションを漸進的に実行する。この現在の状態を、イーサリアムの世界の標準的な「バージョン」として受け入れているのです。

状態には、口座残高、評判、信用協定、物理世界の情報に関連するデータなど、要するに、現在コンピュータで表現できるものなら何でも含めることができる。
現在コンピュータで表現できるものであれば、何でも許容される。この「有効な」という部分が重要で、有効な状態変化よりも無効な状態変化の方がはるかに多いのである。

無効な状態変化とは、例えば、ある口座の残高を減らすが、他の口座の残高が同じだけ増えないというようなものである。
を減らすようなものです。

有効な状態遷移とは、取引によって生じるものである。
トランザクションによって生じるものです。正式には
\begin{eqnarray}
