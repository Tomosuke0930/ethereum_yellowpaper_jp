...
コントラクトエンジニアリングには、特定の便利な動作を可能にするいくつかのパターンがある。そのうちの2つを簡単に説明すると、データフィードと乱数である。
...
データフィード契約は、単一のサービスを提供するものです。それは、イーサリアム内での外部から情報へのアクセスを提供するものです。

この情報の正確性と即時性は保証されておらず、単一のデータフィードにどれだけの信頼を置くことができるかを決定するのは、二次契約作成者（データフィードを利用する契約）の仕事である。

一般的なパターンは、イーサリアム内の単一のコントラクトが、メッセージコールを与えられたときに、外部の現象に関するタイムリーな情報を返信することです。

例えば、ニューヨークの気温のようなものです。これは、ストレージ内のある既知のポイントの値を返すコントラクトとして実装される。もちろん、ストレージ内のこのポイントは正しい温度で維持されなければならない。したがって、パターンの第2部は、外部サーバーがEthereumノードを実行することである。
ノードを実行し、新しいブロックを発見すると直ちに有効なトランザクションを作成し、コントラクトに送信してストレージ内の値を更新する。コントラクトのコードは、当該サーバーに含まれる ID からのみ、そのような更新を受け入れることになる。
...
決定論的なシステムで乱数を提供することは、当然ながら不可能な作業である。しかし、取引時には一般に知ることができないデータを利用することで、擬似乱数で近似することができます。そのようなデータには、ブロックのハッシュ、ブロックのタイムスタンプ、ブロックの受益者アドレスが含まれるかもしれません。悪意のあるマイナーがこれらの値を制御するのを難しくするために、BLOCKHASH
オペレーションを使用して、前の256ブロックのハッシュを擬似乱数として使用する必要があります

このような数字の羅列の場合、ある一定量を加算し、その結果をハッシュ化することが明らかな解決策となる。
