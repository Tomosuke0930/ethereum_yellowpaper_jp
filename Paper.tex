...
状態データベースは、過去のすべての状態ツリー構造を将来にわたって維持することを強制されることはないだろう。各ノードの年齢を維持し、最終的には十分新しいものでもチェックポイントでもないノードを破棄する。チェックポイント、または特定のブロックの状態ツリーをトラバースすることを可能にするデータベース内のノードのセットは、ブロックチェーン全体の任意の状態を取得するために必要な計算量に最大制限を設けるために使用される可能性があります。

ブロックチェーンの統合は、クライアントが完全な採掘ノードとして機能するためにダウンロードする必要があるブロックの量を減らすために使用される可能性があります。
ある時点のツリー構造の圧縮アーカイブ（おそらく10,000ブロックに1つ）をピアネットワークで維持し、ジェネシスブロックを効果的に再作成することが可能である。これにより、ダウンロードする量を1つのアーカイブに加え、ブロックのハード的な最大制限に減らすことができます。

最後に、ブロックチェーンの圧縮はおそらく実施可能である：ある一定量のブロックでトランザクションを送信/受信していない状態トライのノードは捨てられ、Ether-leakageと状態データベースの成長の両方が減少する可能性がある。
...
一般化された状態遷移の関数では、分割して統治を行う戦略を適用するためにトランザクションを分割し並列化することが困難になる。未対策のままでは、システムの動的な価値範囲は基本的に固定されたままであり、平均的な取引額が増加するにつれて、価値の低い取引は無視され、メインの台帳に含めることが経済的に無意味になる。

しかし、よりスケーラブルなプロトコルを提供するために、潜在的に利用できる可能性のある戦略がいくつか存在する。

軽量な小さなチェーンをメインブロックに統合するか、あるいは（プルーフ・オブ・ワークを通じて）小さな取引セットを段階的に組み合わせて結合し、メインブロックを構築することで、何らかの形で階層構造を実現し、取引の組み合わせとブロック構築を並列化することができるかもしれません。

並列性は、優先順位をつけた並列ブロックチェーンのセットから生まれる可能性もあり、各ブロックを統合し、重複する取引や無効な取引はそれに応じて排除される。

最後に、検証可能な計算が一般的に利用可能で十分に効率的であれば、プルーフ・オブ・ワークを最終状態の検証として利用できるようになる可能性がある。
