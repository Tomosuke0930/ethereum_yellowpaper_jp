...
\begin{abstract}
    ブロックチェーンパラダイムは、暗号化された安全な取引と結びついたとき、多くのプロジェクトを通じてその有用性を実証しており、ビットコインは最も注目すべきものの1つです。
    このようなプロジェクトは、分散型だがシングルトンの計算資源上の単純なアプリケーションと見なすことができる。
    このパラダイムを、共有された状態(shared-state)を持つトランザクショナル・シングルトン・マシンと呼ぶことができる。
  
    Ethereumはこのパラダイムを一般化した形で実装しています。さらに、このようなリソースを複数用意し、それぞれが個別の状態と操作コードを持つが、メッセージパッシングのフレームワークを通じて他のリソースと相互作用することが可能である。
  
    本稿では、イーサリアムの設計、実装上の問題点、提供される機会、そして我々が想定する将来のハードルについて議論する。
\end{abstract}
