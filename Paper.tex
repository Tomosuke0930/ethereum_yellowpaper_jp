...
正規のブロックチェーンは、ブロックツリー全体を通してルートからリーフに至るパスである。どのパスであるかについてコンセンサスを得るために、概念的には、最も多くの計算が行われたパス、つまり最も重いパスを特定します。最も重いパスの判断材料は、明らかにリーフのブロック番号で、これはパスの中の未採掘の生成ブロックを除いたブロック数に相当します。パスが長ければ長いほど、そのリーフに到達するまでに費やされた採掘の総労力は大きくなる。これは、ビットコイン由来のプロトコルで採用されているような既存の方式に似ています。
ブロックヘッダにはディフィカルティが含まれているため、ヘッダだけで計算を検証することができる。どのブロックもチェーンの総計算量または総難易度に貢献する。
したがって、ブロックBのtotal difficultyを再帰的に定義する。
このように、あるブロックBが与えられたとき、Btはその合計difficulty、B0はその親ブロック、Bdはそのdifficultyである。
\begin{eqnarray}
...
このように、ブロックBが与えられたとき、Btはその総難易度、B 0 は はその親ブロック、Bdはその難易度である。
